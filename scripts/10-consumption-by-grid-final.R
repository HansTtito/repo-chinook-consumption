# ================================================================
# ESTIMACIÓN DE CONSUMO DE CHINOOK POR GRILLAS - ISOTOPES
# Usando dieta con isótopos músculo 7F para edades 1-2
# ================================================================
...
# Configurar qué procesar (MODIFICAR SEGÚN NECESIDAD)
config <- list(
  años_procesar = 2015:2023,  # Para testing, cambiar por 2015:2023 para completo
  edades_procesar = 1:4,      # Para testing, cambiar por 1:4 para completo  
  max_grillas = NULL,         # Para testing, cambiar por NULL para todas
  verbose = TRUE              # Mostrar progreso detallado
)

cat("=== CONFIGURACIÓN ===\n")
cat("IMPORTANTE: Usando dieta ISOTOPES (músculo 7F para edades 1-2)\n")
...
# ================================================================
# PASO 2: EXTRAER VALORES DE P POR AÑO/EDAD
# ================================================================

cat("=== PASO 2: EXTRAYENDO VALORES DE P ===\n")

# Crear tabla de p-values
...
# ================================================================
# PASO 3: PREPARAR ESTRUCTURA DE RESULTADOS
# ================================================================

cat("=== PASO 3: PREPARANDO ESTRUCTURA DE RESULTADOS ===\n")

# Estructura: resultado_final$año_XXXX$edad_X
...
cat("Estructura creada para", length(años_disponibles), "años y", 
    length(config$edades_procesar), "edades\n\n")

# ================================================================
# PASO 4: PROCESAMIENTO PRINCIPAL
# ================================================================

cat("=== PASO 4: PROCESAMIENTO PRINCIPAL ===\n")
...
for(i in 1:nrow(p_values_table)) {
  ...
  # Extraer información de esta combinación
  ...
  if(config$verbose) {
    ...
    cat("P-value:", round(p_value_actual, 4), "\n")
  }
  
  # ==============================================================
  # PASO 4.1: OBTENER OBJETO BIOENERGÉTICO BASE
  # ==============================================================
  ...
  # ==============================================================
  # PASO 4.2: OBTENER GRILLAS DE TEMPERATURA PARA ESTE AÑO
  # ==============================================================
  
  # año_str <- as.character(año_actual)
  año_str <- as.character(año_actual - 1)  # año real de consumo
  ...
  for(j in 1:n_grillas_procesar) {
    ...
    # ==============================================================
    # PASO 4.4: EJECUTAR MODELO BIOENERGÉTICO
    # ==============================================================
    ...
    tryCatch({
      ...
      # ==============================================================
      # PASO 4.5: EXTRAER CONSUMO TOTAL Y POR PRESAS
      # ==============================================================
  ...
  # ==============================================================
  # PASO 4.6: GUARDAR RESULTADOS DE ESTA COMBINACIÓN AÑO/EDAD
  # ==============================================================
...
cat("Resultado guardado: resultado_consumo_grillas_ISOTOPES.rds\n")

# Resumen de procesamiento
cat("\n=== RESUMEN FINAL ===\n")
cat("Combinaciones procesadas:", contador_completados, "/", total_combinaciones, "\n")
cat("Errores:", contador_errores, "\n")
cat("Años procesados:", paste(años_disponibles, collapse = ", "), "\n")
cat("Edades procesadas:", paste(config$edades_procesar, collapse = ", "), "\n")

# ================================================================
# FUNCIONES PARA MAPAS DE CONSUMO DE CHINOOK POR GRILLAS
# ================================================================
...
# ================================================================
# FUNCIÓN PRINCIPAL: PLOT CONSUMO MAPA
# ================================================================
...
plot_consumo_mapa <- function(datos, año, edad, tipo = "total", 
  ...
  "
  Crea mapas de consumo de Chinook por grillas
  
  Args:
    datos: resultado_final (lista estructurada)
    año: Año a graficar (ej: 2015)
    edad: Edad a graficar (ej: 1)
    tipo: 'total', 'anchoveta', 'sardina', 'otros', o 'presas' (todos)
    titulo: Título personalizado (opcional)
    escala_colores: 'viridis', 'plasma', 'magma', 'inferno'
    mostrar_stats: Mostrar estadísticas en subtitle
    verbose: Mostrar mensajes informativos
    
  Returns:
    Gráfico ggplot (o lista de gráficos si tipo = 'presas')
  "
  
  if(verbose) cat("=== CREANDO MAPA DE CONSUMO (ISOTOPES) ===\n")
  ...
  # ==============================================================
  # PROCESAMIENTO SEGÚN TIPO DE MAPA
  # ==============================================================
  
  if(tipo == "presas") {
    # Crear múltiples mapas (uno por presa)
    mapas_presas <- list()
...
# ================================================================
# FUNCIÓN AUXILIAR: CREAR MAPA INDIVIDUAL
# ================================================================
...
crear_mapa_individual <- function(datos_grillas, año, edad, tipo, 
  ...
  # ==============================================================
  # SELECCIÓN DE VARIABLE Y CONFIGURACIÓN
  # ==============================================================
  
  # Determinar variable y configuración según tipo
  ...
  # # Crear subtitle con estadísticas
  # if(mostrar_stats) {
  #   subtitle <- paste0(
  #     "Año ", año, " | ", stats$n_grillas, " grillas | ",
  #     "Promedio: ", round(stats$consumo_medio, 1), " kg | ",
  #     "Total: ", round(stats$consumo_total, 0), " kg"
  #   )
  # } else {
  #   subtitle <- paste("Año", año, "|", stats$n_grillas, "grillas | Dieta edades 1-2: isótopos músculo 7F")
  # }

  # DESPUÉS:
  año_consumo <- año - 1  # año real de consumo
  ...
  if(mostrar_stats) {
      ...
      subtitle <- paste("Año", año_consumo, "|", stats$n_grillas, 
                        "grillas | Dieta edades 1-2: isótopos músculo 7F")
  }
  # ==============================================================
  # CREACIÓN DEL MAPA
  # ==============================================================
  
  # Determinar función de escala de colores
  ...
  mapa <- ggplot() +
    ...
    labs(
      ...
      x = "Longitud",
      y = "Latitud",
      caption = "Dieta edades 1-2: isótopos músculo 7F | Edades 3-5: estómagos"
    ) +
  ...
  # Agregar información de estadísticas como atributo
...
# ================================================================
# FUNCIÓN DE CONVENIENCIA: COMPARAR EDADES
# ================================================================

plot_comparar_edades <- function(datos, año, edades = 1:4, tipo = "total",
                                ncol = 2, titulo_general = NULL) {
  "
  Crea múltiples mapas para comparar edades en el mismo año
  
  Args:
    datos: resultado_final (lista estructurada)
    año: Año a graficar
    edades: Vector de edades a comparar
    tipo: Tipo de consumo ('total', 'anchoveta', etc.)
    ncol: Número de columnas en la grilla
    titulo_general: Título general para todos los mapas
    
  Returns:
    Lista de mapas ggplot o objeto patchwork combinado
  "
  
  cat("=== COMPARANDO EDADES PARA AÑO", año, "(ISOTOPES) ===\n")
  
  # Manejar caso especial de "presas"
  if(tipo == "presas") {
    cat("Tipo 'presas' detectado - creando mapas separados por presa\n")
    ...
    for(presa in c("anchoveta", "sardina", "otros")) {
      ...
      for(edad in edades) {
        ...
        if(año_key %in% names(datos) && edad_key %in% names(datos[[año_key]])) {
          ...
          # Crear mapa individual para esta presa
      ...
      # Combinar mapas de esta presa
      if(length(mapas_edad_presa) > 0) {
        if(is.null(titulo_general)) {
          # titulo_presa <- paste("Consumo de", tools::toTitleCase(presa), "- Año", año, "(ISOTOPES)")
    ...
    # Caso normal: un tipo específico
    mapas_edades <- list()
    ...
    if(is.null(titulo_general)) {
      # titulo_general <- paste("Comparación de Consumo por Edades - Año", año, "(ISOTOPES)")
...
# ================================================================
# FUNCIÓN AUXILIAR: EXTRAER ESTADÍSTICAS
# ================================================================

extraer_estadisticas_consumo <- function(datos, año, edad, tipo = "total") {
  "
  Extrae estadísticas de consumo sin crear el mapa
  
  Returns:
    Lista con estadísticas del consumo
  "
...
cat("\n=== EJEMPLOS DE USO ===\n")
cat("# Cargar resultados guardados:\n")
cat("resultado_final <- readRDS('data/data_raw/spatial-consumption/resultado_consumo_grillas_ISOTOPES.rds')\n\n")
resultado_final <- readRDS('data/data_raw/spatial-consumption/resultado_consumo_grillas_ISOTOPES.rds')
cat("# Comparar edades para un año:\n")
cat("plot_comparar_edades(resultado_final, 2015, edad = 1:4, tipo = 'total')\n\n")
plot_comparar_edades(resultado_final, 2015, edad = 1:4, tipo = 'total')
cat("# Mapa individual:\n")
cat("plot_consumo_mapa(resultado_final, año = 2015, edad = 1, tipo = 'total')\n\n")
plot_consumo_mapa(resultado_final, año = 2015, edad = 1, tipo = 'total')
cat("# Extraer estadísticas:\n")
cat("extraer_estadisticas_consumo(resultado_final, año = 2015, edad = 1)\n\n")

cat("# Mapas por presa:\n")
cat("plot_comparar_edades(resultado_final, 2015, edad = 1:4, tipo = 'presas')\n\n")
plot_comparar_edades(resultado_final, 2015, edad = 1:4, tipo = 'presas')

cat("¡Procesamiento completado (ISOTOPES)!\n")